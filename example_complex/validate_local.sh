...
git init
dvc init
dvc remote add -d ${REMOTE_NAME} http://localhost:8080/remote?remote=0
dvc remote modify ${REMOTE_NAME} ssl_verify false
## Add and push the data
dvc add $DATASET_FOLDER
dvc push -v $DATASET_FOLDER > report_local_${REMOTE_NAME_FILE}_push.log 2>&1

## Download the data and check
rm -rf $DATASET_FOLDER
rm -rf .dvc/cache
rm -rf .dvc/tmp
dvc pull -v $DATASET_FOLDER > report_local_${REMOTE_NAME_FILE}_pull.log 2>&1

##Try to push again
dvc push -v $DATASET_FOLDER > report_local_${REMOTE_NAME_FILE}_push_retry.log 2>&1
